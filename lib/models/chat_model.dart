...
class ChatModel {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'deletedBy': deletedBy,
      'deletedAt': deletedAt.map((key, value) => MapEntry(key, value != null ? Timestamp.fromDate(value) : null)),
      'lastSeenBy': lastSeenBy.map((key, value) => MapEntry(key, value != null ? Timestamp.fromDate(value) : null)),
  ...
  static ChatModel fromMap(Map<String, dynamic> map) {
    ...
    if (map['lastSeenBy'] != null) {
      ...
      lastSeenMap = rawLastSeen.map((key, value) => MapEntry(key, (value as Timestamp?)?.toDate()));
    }
    ...
    if (map['deletedAt'] != null) {
      ...
      deletedAtMap = rawDeletedAt.map((key, value) => MapEntry(key, (value as Timestamp?)?.toDate()));
    }
  ...
  bool isMessageSeen(String currentUserId, String otherUserId) {
    if (lastMessageSenderId == currentUserId) {
      ...
      if (otherUserLastSeen != null && lastMessageTime != null) {
        return otherUserLastSeen.isAfter(lastMessageTime!) || otherUserLastSeen.isAtSameMomentAs(lastMessageTime!);
      }
    }
    return false;
  }
